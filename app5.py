...
uploaded_file = st.file_uploader("Sube el archivo de Excel con las hojas 'Jugadores' y 'Partidos'", type="xlsx")
...
if uploaded_file:
    ...
    df["puntos"] = df["goles"] * 3 + df["asistencias"] * 1 + df["valla_invicta"] * 2 - df["tarjetas_amarillas"] - df["tarjetas_rojas"] * 2 + df["Penales_Atajados"] * 3
    ...
    resumen_goles["Resultado"] = resumen_goles.apply(lambda row: "Empate" if row["Azul"] == row["Amarillo"] else ("Azul" if row["Azul"] > row["Amarillo"] else "Amarillo"), axis=1)
    ...
    fig, ax = plt.subplots()
    ax.bar(conteo_menciones["Jugador"], conteo_menciones["Menciones"], color="purple")
    ax.set_title("Regularidad del Jugador (Número de Menciones)")
    ax.set_ylabel("Menciones en Rankings")
    ax.tick_params(axis='x', rotation=90)
    st.pyplot(fig)
    ...
    st.dataframe(mvp)
    fig, ax = plt.subplots()
    ax.bar(mvp["Jugador"], mvp["MVP_Score"], color="darkorange")
    ax.set_title("Jugador Más Valioso del Año (MVP)")
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        goleadores.to_excel(writer, sheet_name="Goleadores", index=False)
        asistencias.to_excel(writer, sheet_name="Asistencias", index=False)
        amarillas.to_excel(writer, sheet_name="Amarillas", index=False)
        rojas.to_excel(writer, sheet_name="Rojas", index=False)
        autogoles.to_excel(writer, sheet_name="Autogoles", index=False)
        rendimiento.to_excel(writer, sheet_name="Valla", index=False)
        penales.to_excel(writer, sheet_name="Penales Atajados", index=False)
        df_resumen.to_excel(writer, sheet_name="Jugador de la Fecha", index=False)
        df_evolutivo.to_excel(writer, sheet_name="Evolución", index=False)
        df_acumulado.to_excel(writer, sheet_name="Ranking Anual", index=False)
        resumen_goles.to_excel(writer, sheet_name="Goles por Fecha", index=False)
        total_goles.to_excel(writer, sheet_name="Resumen Anual", index=False)
        df_top3.to_excel(writer, sheet_name="Top 3 Última Fecha", index=False)
        conteo_menciones.to_excel(writer, sheet_name="Regularidad", index=False)
        mvp.to_excel(writer, sheet_name="Ranking MVP", index=False)

    output.seek(0)
    st.download_button(
        label="📥 Descargar Excel con estadísticas",
        data=output,
        file_name="estadisticas_legendarios_fc.xlsx",
